...
const gitlet = {
  ...
  commit: function(opts) {
    ...
    if (refs.hash("HEAD") !== undefined &&
      ...
      // Otherwise, do the commit.
        ...
        let m = merge.isMergeInProgress() ? files.read(files.gitletPath("MERGE_MSG")) : opts.m;

        // Write the new commit to the `objects` directory.
        let commitHash = objects.writeCommit(treeHash, m, refs.commitParentHashes());
  ...
  checkout: function(ref, _) {
    ...
    // pointing at that hash.
      ...
      // Otherwise, perform the checkout.
        ...
        let isDetachingHead = objects.exists(ref);
  ...
  push: function(remote, branch, opts) {
    ...
    // Abort if `remote` not recorded in config file.
      ...
      let remotePath = config.read().remote[remote].url;
      let remoteCall = util.onRemote(remotePath);
  ...
  clone: function(remotePath, targetPath, opts) {
    ...
    // Otherwise, do the clone.
      ...
      util.onRemote(targetPath)(function() {
        ...
        let remoteHeadHash = util.onRemote(remotePath)(refs.hash, "master");
  ...
  update_index: function(path, opts) {
    ...
    let pathFromRoot = files.pathFromRepoRoot(path);
    let isOnDisk = fs.existsSync(path);
    let isInIndex = index.hasFile(path, 0);
  ...
  update_ref: function(refToUpdate, refToUpdateTo, _) {
    ...
    let hash = refs.hash(refToUpdateTo);
    ...
    // that is not a commit.
      ...
      let branch = refs.terminalRef(refToUpdate);
...
// command.
let runCli = module.exports.runCli = function(argv) {
